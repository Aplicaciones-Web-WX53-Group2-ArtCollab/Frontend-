...
<template>
  <div class="popularity-container flex flex-column md:flex-row gap-5 mt-5 mx-2" aria-label="Sección de libros populares">
    <div class="popular-per-genre">
      <h3 class="title flex flex-column md:flex-row md:justify-content-between h-4rem">
        ...
        <pv-dropdown v-model="selectedGenre" :options="genres" :placeholder="$t('select_genre')" optionLabel="name" optionValue="id" @change="onGenreChange" class="border-0 pv-dropdown w-10rem  lg:w-14rem md:w-auto" aria-label="Seleccionar género de libro"/>
      </h3>
      <div class="books flex  p-4  flex-column gap-5">
